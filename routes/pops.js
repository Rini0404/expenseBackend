...
router.post("/", fileUpload({ createParentPath: true }), async function (req, res) {
    ...
    if(!uploadSuccess) {
        res.json({ success: uploadSuccess })
        .end();
        return;
    }
    ...
    res.json({ success: uploadSuccess, recent_upload: popUUID, thumb: `pops/${popUUID}/thumb.jpg`, pops: (await getPops()).map(vidName => `pops/${vidName}`) }).end();
